...
import { useAdmissionCandidatePage } from "../hooks/useAdmissionCandidatePage";
import type {
  AdmissionCandidate,
  CandidateGender,
...
import { AdmissionCandidateMetadataModal } from "./modals/AdmissionCandidateMetadataModal";
...
export function AdmissionCandidatePage() {
  ...
  const { state, actions, flags, bulkUpload } = useAdmissionCandidatePage();
  const {
    candidates,
    totalItems,
    isLoading,
      ...
      <Form layout="vertical" size="middle">
        ...
        <Form.Item label="State" style={{ marginBottom: 12 }}>
          <Select
            ...
            value={stateFilter}
            onChange={(val) => handleStateFilterChange(val as number | undefined)}
            style={{ width: "100%" }}
  ...
  const handleTableChange = (
    _: unknown,
    __: unknown,
    sorter: SorterResult<AdmissionCandidate> | SorterResult<AdmissionCandidate>[],
  ) => {
      ...
      render: (_: unknown, record: AdmissionCandidate) => {
        const menuItems = [
          ...
          },
          {
            ...
            icon: <EditOutlined />,
            onClick: () => handleOpenMetadata(record.id),
          },
          {
            key: "offer",
            label: (
  ...
  return (
    <Flex vertical gap={24} style={{ width: "100%", maxWidth: 1280, margin: "0 auto" }}>
      ...
      <Flex gap={12} align="center" wrap="wrap">
        <Typography.Text type="secondary">Admission cycle</Typography.Text>
        <Select
      ...
      <AdmissionCandidateDetailDrawer
        ...
        onEditMetadata={() => {
          if (drawerCandidateId !== null) {
            handleOpenMetadata(drawerCandidateId);
          }
        }}
        onOffer={(c) => {
          handleCloseDrawer();
