...
export function AdmissionCandidateFormModal({
  ...
  const programLabel = state.createResult?.application
    ? actions.resolveProgramLabel(state.createResult.application.appliedProgramId)
    : "—";
    ...
    <Modal
      title={
        <Flex align="center" gap={8} wrap="wrap">
          ...
          <ConditionalRenderer when={!flags.isResultStep && !!state.cycleStatusLabel}>
          ...
          <Form form={form} layout="vertical">
            ...
            >
              <Select
                ...
                placeholder={ADMISSION_CANDIDATE_CREATE_UI_COPY.appliedProgramPlaceholder}
                options={state.programOptions}
                notFoundContent={
                  state.isProgramsLoading ? "Loading programs…" : "No programs found"
                }
              />
            </Form.Item>
              ...
              <Form.List name="jambScores">
                {(fields, { add, remove }) => (
                  <Flex vertical gap={8} style={{ marginTop: token.paddingSM }}>
                    ...
                    <Button
                      type="dashed"
                      onClick={() => add({ subjectId: undefined, score: undefined })}
                      icon={<PlusOutlined />}
