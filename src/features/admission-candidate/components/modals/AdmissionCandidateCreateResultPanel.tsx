...
export function AdmissionCandidateCreateResultPanel({
  ...
  return (
    <Flex vertical gap={16}>
      ...
      <Card size="small" title={ADMISSION_CANDIDATE_CREATE_UI_COPY.resultApplication}>
        {application ? (
          <Flex vertical gap={4}>
            <Typography.Text>{programLabel}</Typography.Text>
            <Typography.Text type="secondary">
              Status: {formatApplicationStatusLabel(application.applicationStatus)}
            </Typography.Text>
          </Flex>
        ) : (
      ...
      <Card size="small" title={ADMISSION_CANDIDATE_CREATE_UI_COPY.resultJambScores}>
        <Typography.Text>
          {scoreCount > 0
