...
export function BillableEventFormModal({
  ...
  const {
    state: { isEditMode, formError, isSubmitting, catalogEntry },
    actions: {
      handleSubmit,
      handleCancel,
      handleCodeChange,
        ...
        >
          {isEditMode && target ? (
            <Form.Item label="Fee type" tooltip={BILLABLE_EVENT_TOOLTIPS.feeType}>
              <Input
                value={
                  codeOptions.find((option) => option.value === target.code)
                    ?.label ?? target.name
                }
                disabled
              />
          ...
          <Typography.Text
            strong
            style={{ display: "block", marginBottom: 12, fontSize: token.fontSize }}
          >
            When students pay
          </Typography.Text>
