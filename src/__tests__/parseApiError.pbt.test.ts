...
const constraintViolationErrorArb = fc.record({
  type: fc.constant(
    ApiErrorType.ConstraintViolation as typeof ApiErrorType.ConstraintViolation,
  ),
  title: nonEmptyString,
    ...
    fc.assert(
      fc.property(validationErrorArb, (body) => {
        ...
        const expectedKeys = new Set(
          body.violations.map((v) => v.propertyPath),
        );
        const actualKeys = new Set(Object.keys(result.fieldErrors));
...
describe("Property 8: Fallback on invalid ApiErrorResponse.error", () => {
  it("falls back to ApiErrorResponse.message and errorFields when error field is absent", () => {
    ...
    fc.assert(
      fc.property(
        nonEmptyString,
        fc.option(fc.dictionary(nonEmptyString, nonEmptyString), {
          nil: undefined,
        }),
        (message, errorFields) => {
          const input = {
    ...
    fc.assert(
      fc.property(
        ...
        nonEmptyString.filter(
          (s) => !(Object.values(ApiErrorType) as string[]).includes(s),
        ),
        fc.option(fc.dictionary(nonEmptyString, nonEmptyString), {
          nil: undefined,
        }),
        (message, unknownType, errorFields) => {
          const unrecognisedBody = JSON.stringify({
            type: unknownType,
            detail: "x",
            status: 400,
            title: "x",
